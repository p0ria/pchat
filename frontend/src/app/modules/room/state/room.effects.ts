...
export class RoomEffects {
  ...
  createRoom$: Observable<Action> = this.actions$.pipe(
    ofType(roomActions.RoomActionTypes.CreateRoom),
    mergeMap((action: roomActions.CreateRoom) =>
      this.roomService.createRoom(action.payload)),
    map((room: Room) => new roomActions.CreateRoomSuccess(room)),
    catchError(err => of(new roomActions.CreateRoomFail(err.message)))
  )
}
